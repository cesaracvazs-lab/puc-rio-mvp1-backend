...
def atualizar_cliente(query: ClienteAtualizarSchema):    
    ...
    try:
        ...
        campos_enviados = query.model_fields_set

        if 'nome' in campos_enviados and query.nome:
            cliente.nome = query.nome

        if 'email' in campos_enviados and query.email:
            cliente.email = query.email

        if 'assinatura_id' in campos_enviados and query.assinatura_id is not None:
            assinatura = session.query(Assinatura).filter(Assinatura.id == query.assinatura_id).first()
            if not assinatura:
                return {"error": "Assinatura não encontrada"}, 404

            cliente.assinatura_id = query.assinatura_id
        ...
        if 'data_nascimento' in campos_enviados:
            if query.data_nascimento:
                try:
                    cliente.data_nascimento = datetime.strptime(query.data_nascimento, '%d/%m/%Y').date()
